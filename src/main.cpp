...
#if defined(NDEBUG)
# error "SaveCoin cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight, int64_t nFees)
{
    ...
    // Subsidy is cut in half every 500,000 blocks which will occur approximately every 202 days.
    nSubsidy >>= (nHeight / Params().SubsidyHalvingInterval());

    return nSubsidy + nFees;
}

static const int64_t nTargetTimespan = 140; // 140 seconds
static const int64_t nTargetSpacing = 35; // 35 seconds
...
void ThreadScriptCheck() {
    RenameThread("SaveCoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
   ...
   CExtDiskTxPos pos(CDiskTxPos(pindex->GetBlockPos(), GetSizeOfCompactSize(block.vtx.size())), pindex->nHeight);
    ...
    if (block.vtx[0].GetValueOut() > GetBlockValue(pindex->nHeight, nFees))
	{
				...
				// Add workaround for first non-genesis block to be accepted.
				if (block.vtx[0].GetValueOut() == 1999989700000000)  
				{
				if (nCheckNonGenesisBlock == 0) { nCheckNonGenesisBlock = 1;} // Only allow this exception happen on first block, reject any other blocks with same value after first block.
				else { return state.DoS(100, error("ConnectBlock() : coinbase pays too much (actual=%d vs limit=%d)", block.vtx[0].GetValueOut(), GetBlockValue(pindex->nHeight, nFees)), REJECT_INVALID, "bad-cb-amount");}
